...
local function buildPayload(src, action, message, opts)
    ...
    if opts.extra and type(opts.extra) == "table" then
        -- Stabil & lesbar: erst wichtige Keys in fixer Reihenfolge, dann Rest alphabetisch.
        ...
        for i = 1, #preferredOrder do
            local k = preferredOrder[i]
            local v = opts.extra[k]
            if v ~= nil then
                used[k] = true
        ...
        local remainingKeys = {}
        for k, _ in pairs(opts.extra) do
            if not used[k] then
                remainingKeys[#remainingKeys + 1] = k
            end
        end
        table.sort(remainingKeys, function(a, b) return tostring(a) < tostring(b) end)
        for i = 1, #remainingKeys do
            local k = remainingKeys[i]
            parts[#parts + 1] = tostring(k) .. "=" .. tostring(opts.extra[k])
        end

        if #parts > 0 then
            -- Wenn msg leer ist, gib NUR die Extras aus (ohne führendes "- |").
            if msg == "" then
