...
local function pickLock(entity)
	...
	if success then
		...
		local doorName = door.name or ('door#' .. tostring(door.id))
		local action = locale('lockpick_log_action')
		local message = locale('lockpick_success_log', doorName)

		-- Optionales Logging (nicht crashen, falls DFLogs nicht geladen ist)
		if DFLogs and DFLogs.Log then
			DFLogs.Log(action, message)
		end
	else
		-- Lockpicking fehlgeschlagen (kein State-Change). Dietrich-Bruch wird separat geloggt (rand == 1).
		local doorName = door.name or ('door#' .. tostring(door.id))
		local action = locale('lockpick_log_action')
		local message = locale('lockpick_failed_log', doorName)
	...
	if rand == 1 then
		...
		local doorName = door.name or ('door#' .. tostring(door.id))
		local action = locale('lockpick_log_action')
		local message = locale('lockpick_broke_log', doorName)
