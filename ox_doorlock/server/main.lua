...
local function stringifyLogValue(v)
	local t = type(v)
	...
	-- Many FiveM values can be userdata (e.g. vector3, lua_rapidjson_array).
	-- Prefer encoding to JSON so our diff compares *content* instead of pointer addresses.
	local okEnc, encoded = pcall(json.encode, v)
	if okEnc and encoded then
		if #encoded > 600 then
			return encoded:sub(1, 600) .. '…'
		end
		return encoded
	end

	-- Fallback for userdata / unknown types
	local okStr, str = pcall(tostring, v)
...
local function buildChangesFromUpdate(oldDoor, update)
	if type(oldDoor) ~= 'table' or type(update) ~= 'table' then return nil end

	local changes = {}

	for k, v in pairs(update) do
		if k ~= 'id' then
			local newVal = (v ~= '' and v or nil)
			local oldVal = oldDoor[k]

			-- basic equality; for tables/userdata we stringify
			local equal = (oldVal == newVal)
			if not equal then
				local oldStr = stringifyLogValue(oldVal)
				local newStr = stringifyLogValue(newVal)
				if oldStr ~= newStr then
					changes[#changes + 1] = ('%s: %s -> %s'):format(tostring(k), oldStr, newStr)
				end
			end
		end
	end
...
local function buildChangesFull(oldDoor, newDoor)
	if type(oldDoor) ~= 'table' or type(newDoor) ~= 'table' then return nil end
	...
	local keys = {}
	for k in pairs(oldDoor) do
		if not ignore[k] then keys[k] = true end
	end
	for k in pairs(newDoor) do
		if not ignore[k] then keys[k] = true end
	end
...
exports('editDoor', function(id, data)
	...
	if door then
		local oldName = door.name
		local changes = buildChangesFromUpdate(door, data)
		...
		-- Log: Doorlock bearbeitet (per Script/Export)
		local inv = GetInvokingResource()
		if DFLogs and DFLogs.Log then
			local doorName = door.name or oldName or tostring(id)
			local msg = doorName
			if changes then msg = msg .. ' | ' .. locale('doorlock_changes_log', changes) end

			DFLogs.Log(locale('door_edit_action'), msg, {
				allowNoSource = true,
				resource = cache.resource,
				player = inv and ('SYSTEM@' .. inv) or 'SYSTEM',
				extra = { doorId = id }
			})
		end
	end
...
local function setDoorState(id, state, lockpick)
	...
	if door then
		-- Log: Doorstate per Script (Exports / anderer Invoker)
		local inv = GetInvokingResource()
		if inv and inv ~= cache.resource and DFLogs and DFLogs.Log then
			local doorName = door.name or tostring(id)
			local stateLabel = state == 1 and locale('locked_door') or locale('unlocked_door')
			DFLogs.Log(locale('doorstate_script_action'), locale('doorstate_script_log', doorName, stateLabel, inv), {
				allowNoSource = true,
				resource = cache.resource,
				player = 'SYSTEM@' .. inv,
				extra = { doorId = id, state = state }
			})
		end
...
RegisterNetEvent('ox_doorlock:editDoorlock', function(id, data)
	-- capture source early (important when awaiting DB calls)
	local src = source

	if IsPlayerAceAllowed(src, 'command.doorlock') then
		local prevDoor = id and doors[id] or nil
		local prevName = prevDoor and prevDoor.name or (id and tostring(id) or nil)
		local changes
		...
		if data then
			...
			-- Avoid accidental coordinate drift from client/UI payloads:
			-- When editing an existing door, keep the stored coords (and double-door coords) unless you intentionally change them.
			if id and prevDoor then
				if prevDoor.coords then
					data.coords = prevDoor.coords
				end

				if prevDoor.doors and data.doors then
					for i = 1, 2 do
						if prevDoor.doors[i] and data.doors[i] and prevDoor.doors[i].coords then
							data.doors[i].coords = prevDoor.doors[i].coords
						end
					end
				end
			end
		...
		if id then
			changes = (data and prevDoor) and buildChangesFull(prevDoor, data) or nil
			...
			-- Log: Doorlock bearbeitet / gelöscht (Admin)
			if DFLogs and DFLogs.Log then
				if data then
					local doorName = data.name or tostring(id)
					local msg = doorName
					if changes then msg = msg .. ' | ' .. locale('doorlock_changes_log', changes) end

					DFLogs.Log(src, locale('door_edit_action'), msg, {
						resource = cache.resource,
						extra = { doorId = id }
					})
				else
					DFLogs.Log(src, locale('door_delete_action'), (prevName or tostring(id)), {
						resource = cache.resource,
						extra = { doorId = id }
					})
				end
			...
			-- Log: Doorlock erstellt (Admin)
			if DFLogs and DFLogs.Log then
				DFLogs.Log(src, locale('door_create_action'), (door.name or tostring(door.id)), {
					resource = cache.resource,
					extra = { doorId = door.id }
				})
			end
		end
	end
